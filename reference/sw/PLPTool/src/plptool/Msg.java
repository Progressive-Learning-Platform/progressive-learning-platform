...
public class Msg
{
	...
	static
	{
		...
		try
		{
			PrintStream persistantLog = new PrintStream(new FileOutputStream("log.txt",
					true));
	...
	/**
	 * This function either prints out an error message to stderr or the specified
	 * JTextArea in the output pointer. Error numbers equal or greater to 1024 will always
	...
	private static void logError(PrintStream stream, String errStr, int errorCode,
			Object objIdentifier)
	{
		try
		{
			if (objIdentifier != null)
				stream.println("[ERROR]" + (errorCode == -1 ? "" : " #" + errorCode)
						+ " " + objIdentifier.toString() + ": "
						+ errStr.replaceAll("<[^>]*>", ""));
			else
				stream.println("[ERROR]" + (errorCode == -1 ? "" : " #" + errorCode)
						+ " " + errStr.replaceAll("<[^>]*>", ""));
	...
	/**
	 * This function either prints out an warning message to stdout or the specified
	 * JTextArea in the output pointer.
	 *
	 * @param warningStr
	 *            Error string to print.
	 * @param objIdentifier
	...
	public static void warning(String warningStr, Object objIdentifier)
	{
		if (!suppressWarnings)
		{
			String pre = "<b><font color=red>[WARNING]</font></b> ";
			String post = warningStr + "<br />";
			printMessageToGUI(pre, objIdentifier, post);
	...
	private static void printMessageToGUI(String pre, Object id, String post)
	{
		try
		{
			if (output != null)
			{
				String center = (id != null) ? id.toString() + ": " : "";
				kit.insertHTML(doc, doc.getLength(), pre + center + post, 0, 0, null);
	 ...
	 *
	 * @param infoStr
	 *            Information string to print out.
	 * @param objIdentifier
	 *            The object invoking this call.
	 */
	public static void info(String infoStr, Object objIdentifier)
	{
		if (!silent)
		{
			String pre = "<font face=\"sans-serif\"><font color=gray>&bull;</font> ";
			String post = infoStr + "</font><br />";
			printMessageToGUI(pre, objIdentifier, post);
	...
	/**
	 * Print out a debug message if the current debug level is lower or equal to the
	 * requested level.
	 *
	 * @param debugStr
	 *            Debug string to print out.
	 * @param requestedDebugLevel
	 *            Debug level requested.
	 * @param objIdentifier
	 *            The object invoking this call.
	 */
	public static void debug(String debugStr, int requestedDebugLevel,
			Object objIdentifier)
	{
		if (requestedDebugLevel <= Constants.debugLevel)
		{
			String pre = "<b><font color=gray>[DEBUG]</font></b> ";
			String post = "<font color=\"#444444\">" + debugStr + "</font><br />";
			printMessageToGUI(pre, objIdentifier, post);
			
			for (PrintStream stream : logStreams)
				logDebug(stream, debugStr, objIdentifier);
		}
	}
	
	private static void logDebug(PrintStream stream, String debugStr, Object objIdentifier)
	{
		if (stream != null)
		{
			try
			{
				if (objIdentifier != null)
					stream.println("[DEBUG] " + objIdentifier.toString() + ": "
							+ debugStr);
				else
			...
			if (!silent)
			{
				...
				if (output != null)
				{
					kit.insertHTML(
							doc,
							doc.getLength(),
							"<font face=monospaced size=12pt>"
									+ msgStr.replace(" ", "&nbsp;") + "</font><br />", 0,
							0, null);
					output.setCaretPosition(doc.getLength());
				}
	...
	/**
	 * Print pre-formatted string (HTML's
	 * 
	 * <pre>
	 * tag)
	 *
			...
			if (!silent)
			{
				...
				if (output != null)
				{
					kit.insertHTML(doc, doc.getLength(),
							"<pre><font face=monospaced size=10pt>" + message
									+ "</font></pre>", 0, 0, null);
					output.setCaretPosition(doc.getLength());
				}
	...
	/**
	 * Append string to the preformatted string buffer with no newline added in the end
	 *
	 * @param appendStr
	 *            String to append
	 */
	...
	public static void printBuffer()
	{
		...
		try
		{
			if (!silent)
			{
				...
				{
					kit.insertHTML(doc, doc.getLength(),
							"<pre><font face=monospaced size=10pt>"
									+ preformattedStringBuffer + "</font></pre>", 0, 0,
							null);
					output.setCaretPosition(doc.getLength());
				}
