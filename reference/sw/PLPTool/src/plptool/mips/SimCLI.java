...
public class SimCLI {
    ...
    public static void simCLCommand(String cmd, plptool.gui.ProjectDriver plp) {
        ...
        if(input.equals("version")) {
            Msg.printPreformattedString(plptool.Text.versionString);
        }
    ...
    public static void cmd_s_arg() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: s <number of instructions>");
        }
        else {
            Msg.lastError = 0;
            ...
            if(steps > Constants.PLP_LONG_SIM) {
                if(!silent) {
                    Msg.printPreformattedString("This might take a while, turning on silent mode.");
                    silent = true;
                }
            ...
            for(int i = 0; i < steps && Msg.lastError == 0 && !breakpoint; i++) {
                ...
                if(core.breakpoints.isBreakpoint(core.visibleAddr))
                    Msg.printPreformattedString("--- stopping at breakpoint: " + PLPToolbox.format32Hex(core.visibleAddr));
            }
            if(steps > Constants.PLP_LONG_SIM) {
                startCount = core.getInstrCount() - startCount;
                Msg.printPreformattedString("--- executed " + startCount + " instructions in " +
    ...
    public static void cmd_wpc() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: wpc <address>");
        } else {
            core.softreset();
    ...
    public static void cmd_j() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: j <address>");
        }
        else {
            core.pc.write(PLPToolbox.parseNum(tokens[1]));
            core.printfrontend();
    ...
    public static void cmd_w() {
        if(tokens.length != 3) {
            Msg.printPreformattedString("Usage: w <address> <data>");
        } else {
    ...
    public static void cmd_fwd() {
        Msg.printPreformattedString("EX->EX: " + core.ex_ex);
        Msg.printPreformattedString("MEM->EX: " + core.mem_ex);
        Msg.printPreformattedString("MEM->EX LW-use: " + core.mem_ex_lw);
    }
    ...
    public static void cmd_pram() {
        ...
        for(int i = 0; i < ioReg.getNumOfModsAttached(); i++) {
            if(ioReg.getType(i) == 0) {
                Msg.printPreformattedString("Attached memory in bus position " + ioReg.getPositionInBus(i));
    ...
    public static void cmd_pram_arg() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: pram <address>");
        } else {
            for(int i = 0; i < ioReg.getNumOfModsAttached(); i++) {
                if(ioReg.getType(i) == 0) {
                    Msg.printPreformattedString("Memory attached in bus position " + ioReg.getPositionInBus(i));
    ...
    public static void cmd_preg_arg() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: preg <address>");
        } else {
    ...
    public static void cmd_pprg() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: pprg <index of memory module on the BUS>");
        } else {
            Msg.printPreformattedString("\nProgram Listing");
            Msg.printPreformattedString("===============");
    ...
    public static void cmd_wbus() {
        if(tokens.length != 3) {
            Msg.printPreformattedString("Usage: wbus <address> <data>");
        } else {
    ...
    public static void cmd_rbus() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: rbus <address>");
        } else {
            long addr = PLPToolbox.tryResolveLabel(tokens[1], asm);
            if(addr > -1) {
                ...
                if(ret != null) {
                    long value = (Long) ret;
                    Msg.printPreformattedString(String.format("0x%08x=", addr) +
    ...
    public static void cmd_enableio_arg() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: enableio <index>");
        } else {
    ...
    public static void cmd_disableio_arg() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: disableio <index>");
        } else {
    ...
    public static void cmd_evalio_arg() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: evalio <index>");
        } else {
    ...
    public static void cmd_cleario() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: cleario <index>");
        } else {
    ...
    public static void cmd_listmods() {
        Msg.printPreformattedString("Registered modules:");
        Object modInfo[][] = ioReg.getAvailableModulesInformation();
        for(int i = 0; i < modInfo.length; i++) {
            Msg.printPreformattedString(i + ": " + modInfo[i][0] + " - " + modInfo[i][3]);
        }
    }

    public static void cmd_addmod() {
        if(tokens.length != 4) {
            Msg.printPreformattedString("Usage: addmod <mod ID> <address> <register file size>");
        } else {
    ...
    public static void cmd_listdmods() {
        Msg.printPreformattedString("Registered dynamic modules:");
        for(int i = 0; i < plptool.dmf.DynamicModuleFramework.getNumberOfClasses(); i++) {
            ...
            Msg.printPreformattedString(i + ":\t" + c.getName());
            Msg.printPreformattedString((sc != null) ? " extends " + sc.getName() : "");
    ...
    public static void cmd_adddmod() {
        if(tokens.length != 4) {
            Msg.printPreformattedString("Usage: adddmod <dynamic mod ID> <start address> <end address>");
        } else {
    ...
    public static void cmd_rmmod() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: rmmod <mod index in the REGISTRY>");
        } else {
    ...
    public static void cmd_attachedmods() {
        Msg.printPreformattedString("Attached modules:");
    ...
    public static void cmd_listpresets() {
        Msg.printPreformattedString("Registered presets:");
        Object[][] presets = plptool.mods.Preset.presets;
        for(int i = 0; i < presets.length; i++) {
            Msg.printPreformattedString(i + ": " + presets[i][0]);
        }
    }

    public static void cmd_loadpreset() {
        if(tokens.length != 2) {
            Msg.printPreformattedString("Usage: loadpreset <index>");
        }
        else {
    ...
    public static void cmd_asm() {
        if(tokens.length < 3) {
            Msg.printPreformattedString("Usage: asm <address> <in-line assembly>");
        } else {
            String iAsm = "";
            long addr;
            ...
            if(inlineAsm.isAssembled() && base > -1) {
                ...
                for(int j = 0; j < inlineAsm.getObjectCode().length; j++) {
                    ...
                    Msg.printPreformattedString(String.format("%08x", addr) +
    ...
    public static void cmd_silent() {
        if(silent) {
            silent = false;
            Msg.printPreformattedString("Silent mode off.");
        } else {
            silent = true;
            Msg.printPreformattedString("Silent mode on.");
        }
    }
    ...
    public static void cmd_cycleaccurate() {
        if(Config.simFunctional) {
            ...
            Msg.printPreformattedString("Functional simulation mode off.");
        } else {
            Config.simFunctional = true;
            Msg.printPreformattedString("Functional simulation mode on.");
        }
    }
        ...
        if(tokens.length == 4) {
            ...
            if(index > -1 && addr > -1 && assertvalue > -1) {
                ...
                if(value != null && (long) value == assertvalue)
                    Msg.printPreformattedString("True");
                else
                    Msg.printPreformattedString("False");
            } else
                Msg.printPreformattedString("False");
        } else
            Msg.printPreformattedString("Usage: assert <module index in the BUS> <address/label> <value>");
    }
    ...
    public static void cmd_assertreg() {
        if(tokens.length == 3) {
            ...
            Msg.printPreformattedString("Usage: assertreg <register address> <value>");
    }
    ...
    public static void cmd_echo() {
        if(tokens.length == 1)
            ...
            Msg.printPreformattedString(input.substring(5, input.length()));
    }
    ...
    public static void cmd_bp() {
        if(tokens.length < 2) {
            ...
            for(int i = 0; i < core.breakpoints.size(); i++) {
                ...
                Msg.printPreformattedString(i + "\t" + PLPToolbox.format32Hex(addr) + (label != null ? " " + label : ""));
            }
    ...
    public static void cmd_jvm() {
        ...
        Msg.printPreformattedString("Free JVM memory:     " + runtime.freeMemory());
        Msg.printPreformattedString("Total JVM memory:    " + runtime.totalMemory());
        Msg.printPreformattedString("Total - Free (Used): "  + (runtime.totalMemory() -  runtime.freeMemory()));
    }
