...
public class Asm extends plptool.PLPAsm {
    ...
    public int preprocess(int index) {
        ...
        Msg.debug("preprocess: splitting...", 4, this);
        ...
        Msg.debug("pp(" + asmIndex + ") begin loop...", 4, this);
        Msg.debug("lines: " + asmLines.length, 4, this);
        ...
        while(i < asmLines.length) {
            ...
            if(commentSplit.length > 0) {
                ...
                Msg.debug(i + ": " + asmLines[i] + " tl: " +
                     asmTokens.length, 5, this);
                Msg.debug("<<<" + asmTokens[0] + ">>>", 5, this);
            } else
                asmTokens = null;
...
// ************************** Assembler Directives *****************************
            ...
            // Include statement
                ...
                if(asmTokens.length != 2) {
                   ...
                   Msg.error(errMsg, Constants.PLP_ASM_DIRECTIVE_SYNTAX_ERROR, this);
                   addBuildError(index, i, errMsg);
                    ...
                    for(int k = 0; k < sourceList.size(); k++) {
                        if(asmTokens[1].equals(sourceList.get(k).getAsmFilePath())) {
                            Msg.warning("preprocess(" + formatHyperLink(curActiveFile, i) + "): " +
            ...
            // .org directive
                ...
                if(asmTokens.length != 2) {
                   ...
                   addBuildError(index, i, errMsg);
                    ...
                    if(entryPoint < 0)
                        entryPoint = curAddr;
                    Msg.debug("curAddr is now " + String.format("0x%08x", curAddr), 5, this);
                }
            }
            ...
            // .text directive
                ...
                if(asmTokens.length < 1 || asmTokens.length > 2) {
                   ...
                   addBuildError(index, i, errMsg);
                    ...
                    if(curAddr < 0) {
                        error++; Msg.error("preprocess(" + formatHyperLink(curActiveFile, i) + "): " +
            ...
            //  symbol table
                ...
                if(asmTokens.length != 3) {
                   ...
                   Msg.error(errMsg, Constants.PLP_ASM_DUPLICATE_LABEL, this);
                   addBuildError(index, i, errMsg);                    
                } else {
                    ...
                    if(val < 0) {
                        ...
                        Msg.error(errMsg, Constants.PLP_NUMBER_ERROR, this);
                        addBuildError(index, i, errMsg);
                    } else {
            ...
            // Text handler
                ...
                if(asmTokens.length < 2) {
                   ...
                   addBuildError(index, i, errMsg);
                    ...
                    Msg.debug("l: " + tString.length + " :" + tString[tString.length - 1], 6, this);
                    ...
                    if(tString[1].charAt(0) == '\"') {
                        ...
                        if(tString[1].charAt(tString[1].length() - 1) != '\"') {
                            ...
                            Msg.error(errMsg, Constants.PLP_ASM_INVALID_STRING, this);
                            addBuildError(index, i, errMsg);
                        }

                        tString[1] = tString[1].substring(0, tString[1].length() - 1);
                    }
                    Msg.debug("pr: " + tString[1] + " l: " + tString[1].length(), 6, this);
...
// ******************************** Instructions *******************************
                ...
                if(instrMap.containsKey(asmTokens[0]) == false) {
                    ...
                    Msg.error(errMsg, Constants.PLP_ASM_INVALID_TOKEN, this);
                    addBuildError(index, i, errMsg);
                }
                Msg.debug("exit i: " + i, 6, this);
                regionMap.add(curRegion);
                curAddr += 4;
            ...
            Msg.error(errMsg, Constants.PLP_GENERIC_ERROR, this);
            addBuildError(index, i, errMsg);
            Msg.printStackTrace(e);
        }

        if(error > 0)
            Msg.error("preprocess: " +
                     error + " error(s).", Constants.PLP_ASM_PREPROCESS_FAILED, this);
        
        Msg.debug("preprocess(" + sourceList.get(index).getAsmFilePath() + "): First pass completed.", 2, this);
    ...
    public int assemble() {
        ...
        while(i < asmLines.length) {
            ...
            skip = false;
            Msg.debug("assemble(file " + sourceList.get(asmFileMap[i]).getAsmFilePath() +
            ...
            for(j = 0; j < asmTokens.length; j++) {
                ...
                Msg.debug(asmTokens[j] + " l:" + asmTokens[j].length(), 15, this);
                
                if(asmTokens[j].length() > 4)  {

                    Msg.debug(asmTokens[j].substring(0, 5), 15, this);
                    ...
                    Msg.debug("pr: " + Integer.toHexString(tValue), 15, this);
                }
            }
		...
		case 8:
                    if(!checkNumberOfOperands(asmTokens, 4, i)) {
                       ...
                       !regs.containsKey(asmTokens[3])) {
                        ...
                        Msg.error(errMsg, Constants.PLP_ASM_INVALID_REGISTER, this);
                ...
                case 3:
                    if(!checkNumberOfOperands(asmTokens, 4, i)) {
                        ...
                        Msg.error(errMsg, Constants.PLP_ASM_INVALID_BRANCH_TARGET, this);
                        addBuildError(fileIndex, lineNumber, errMsg);
                    } else {
        ...
        Msg.debug("assemble(): Assembly completed.", 1, this);
        ...
        if(error > 0)
            ...
            Msg.info("Total statically allocated memory: <b>" + (objectCode.length + bytesSpace / 4) +
                    "</b> words", this);
            Msg.info("Object code and initialized variables: <b>" + objectCode.length + "</b> words", this);
            assembled = true;
        }
    ...
    private boolean checkNumberOfOperands(Object iObj[], int length, int asmLineNum) {
        if(iObj.length != length) {
            ...
            Msg.error(msg, Constants.PLP_ASM_INVALID_NUMBER_OF_OPERANDS, this);
    ...
    public int preSimulationCheck() {
        if(getObjectCode().length == 0)
            return Msg.error("simulate: Empty program.",
